...
def create_test_user(client):
    user_data = {"email": "carlos@gmail.com", "password": "123"}
...
@pytest.fixture
def create_2_test_user(client):
    user_data = {"email": "carlos123@gmail.com", "password": "123"}
    res = client.post("/users/", json=user_data)
    assert res.status_code == 201
    new_user = res.json()
    new_user['password'] = user_data['password']
...
@pytest.fixture
def test_posts(create_test_user, session, create_2_test_user):
    posts_data = [{
        "title": "first title",
        ...
        "title": "4rd title",
        "content": "4rd content",
        "owner_id": create_2_test_user['id']
    }]
