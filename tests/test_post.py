...
def test_get_one_post_not_exist(authorized_client, test_posts):
    res = authorized_client.get(f"/posts/889898")
...
def test_delete_post_success(authorized_client, create_test_user, test_posts):
    res = authorized_client.delete(f"/posts/{test_posts[0].id}")
    assert res.status_code == 204


def test_delete_post_non_exist(authorized_client, create_test_user, test_posts):
    res = authorized_client.delete(f"/posts/-1")
    assert res.status_code == 404


def test_delete_other_user_post(authorized_client, create_test_user, test_posts):
...
def test_update_other_user_post(authorized_client, create_test_user, create_2_test_user, test_posts):
    data = {
        "title": "updated title",
        "content": "updatd content",
        "id": test_posts[3].id
